...
function ubermenu_menu_integration_code( $args , $config_id ){
	...
	$code =
		'<div class="ubermenu-integration-code ubermenu-integration-code-'.$code_id.'">'.
			...
			<p class="ubermenu-sub-desc ubermenu-desc-mini" >'.sprintf( __( 'Click to select, then %s or %s to copy to clipboard', 'ubermenu' ), '<strong><em>&#8984;+c</em></strong>', '<strong><em>ctrl+c</em></strong>' ).'</p>
			<p class="ubermenu-sub-desc ubermenu-desc-understated">'.__( 'Pick the appropriate code and add to your theme template or content where you want the menu to appear.', 'ubermenu' ).'</p>
			<p class="ubermenu-sub-desc ubermenu-sub-desc-manualint"><i class="fas fa-arrow-down"></i> '. __( 'Select a <strong>Theme Location</strong> or <strong>Menu</strong> below to generate the proper code above', 'ubermenu' ). '<i class="fas fa-arrow-up"></i></p>'.
		'</div>';
...
function ubermenu_integration_code_ui( $config_id ){
	...
	$menu_select = '<h4>'.__( 'Integrate Specific Menu', 'ubermenu' ). '</h4>';
	$loc_select = '<h4>'.__( 'Integrate Specific Theme Location', 'ubermenu' ).'</h4>';
	...
	if( is_array( $menus ) ){
		...
		$menu_select.= '<option value="_default">'.__( 'Default', 'ubermenu' ).'</option>';
		foreach( $menus as $menu ){
			$menu_select.= '<option value="'.$menu->term_id.'">'.$menu->name.'</option>';
		}
		$menu_select.= '</select>';

		$menu_select.= '<p class="ubermenu-sub-desc ubermenu-desc-understated">'.__( 'To display a specific menu, select the menu above to generate that code', 'ubermenu' ). '</p>';
	}
	...
	if( is_array( $locs ) ){
		...
		$loc_select.= '<option value="_default">'.__( 'None', 'ubermenu' ).'</option>';
		foreach( $locs as $loc_id => $loc_name ){
			$loc_select.= '<option value="'.$loc_id.'">'.$loc_name.'</option>';
		}
		$loc_select.= '</select>';

		$loc_select.= '<p class="ubermenu-sub-desc ubermenu-desc-understated">'.__( 'To display a specific theme locaton, select the theme location above to generate that code', 'ubermenu' ).'</p>';
	}
...
function ubermenu_pro_instance_settings( $settings , $config_id ){
	...
	$settings[5] = array(
		...
		'type'	=> 'html',
		'desc'	=> '<a target="_blank" class="button button-primary" href="'.ubermenu_sandbox_url().'">'.__( 'View Menu in Sandbox', 'ubermenu' ).'</a> <p><span class="description">' . __('View the menu in a sandbox so you can see how the menu should look with no CSS or JS interference.  Note that PHP interference via filters or actions may still apply.', 'ubermenu' ) .'  <a href="https://sevenspark.com/docs/ubermenu-3/sandbox">'.__( 'Learn more', 'ubermenu' ).'</a></span></p>',
		'group'	=> 'integration',
	...
	$settings[20] = array(
		...
		'desc'	=> __( 'Select the theme locations to activate automatically.  Works with most modularly coded themes.  The above theme locations are specific to your site.' , 'ubermenu' ) .
					'<div class="ubermenu-alert">'.
						'<strong>'.__( 'Are things not looking right after Automatic Integration?', 'ubermenu' ).'</strong>'.
						'<p>'.
							sprintf( __( 'You may have interference from the theme.  Please visit the <a target="_blank" href="%s">Resolving Theme Interference Guide</a> for the next steps', 'ubermenu' ), 'https://sevenspark.com/docs/ubermenu-3/integration/fix-interference' )
						.'</p>'.
					'</div>',
	...
	$settings[22] = array(
		...
		'type'	=> 'checkbox',
		'desc'	=> sprintf( __( '(Automatic Integration Only).  When a mobile device is detected via <a href="%s" target="_blank">wp_is_mobile()</a>, UberMenu will not replace the menu via automatic integration.  By default this includes tablets.  <br/>- <strong>Keep in mind if you are using a cache, you must cache desktop and mobile separately in order for this to work properly</strong>.  <br/>- Remember that UberMenu Advanced Items can only be used within UberMenus, so you likely don\'t want to enable this setting if you are using them.  <br/>- To customize what is considered "mobile", you can use the <a href="%s" target="_blank"><code>ubermenu_is_mobile</code></a> filter', 'ubermenu' ), 'https://codex.wordpress.org/Function_Reference/wp_is_mobile', 'https://sevenspark.com/docs/ubermenu-3/developers/php-api/filters/ubermenu_is_mobile' ),
		'default' => 'off',
		'group'	=> 'integration',
	);
	...
	//<br/><strong>Flex (Justify)</strong> will use Flexbox styling to evenly distribute any extra space in the menu bar evenly over the items.
	$settings[190] = array(
		'name'		=> 'items_align',
		...
		'options' 	=> array(
			'auto'	=> [
				'title' => __( 'Automatic' , 'ubermenu' ),
				'desc' => __( 'Align left/right based on the current language direction LTR/RTL', 'ubermenu' ),
			],
			'left' 	=> [
				'title' => __( 'Left', 'ubermenu' ),
				'desc' => __( 'Align the menu items to the left edge of the menu bar', 'ubermenu' ),
			],
			'center'=> [
				'title' => __( 'Center', 'ubermenu' ),
				'desc' => __( 'Align the menu items to the center of the menu bar', 'ubermenu' ),
			],
			'right'	=> [
				'title' => __( 'Right', 'ubermenu' ),
				'desc' => __( 'Align the menu items to the right edge of the menu bar', 'ubermenu' ),
			],
			'flex'	=> [
				'title' => __( 'Flex (Justify)', 'ubermenu' ),
				'desc' => __( 'Use Flexbox styling to evenly distribute any extra space in the menu bar evenly over the items', 'ubermenu' ),
			]
		),
		'desc'	=> __( 'The alignment of the top level menu items within the menu bar.  Please note that the menu item alignment will only have an effect if the menu bar is wider than the menu items.  ' , 'ubermenu' ),
		'default' 	=> 'auto',
		'group'	=> 'position',
		'customizer'	=> true,
		'customizer_section' => 'menu_bar',
	...
	$settings[195] = array(
		'name' 	=> 'items_vertical_stretch',
		'label' => __( 'Vertical Item Stretch' , 'ubermenu' ),
		'desc'	=> __( 'Stretch all top level items to equal height and center content vertically.  Please note that because this setting uses flexbox, it is not compatible with multi-dimensional item layouts for top level items, such as an image on the left with title and description on the right', 'ubermenu' ),
		'type' 	=> 'checkbox',
		'default' => 'off',
		'group' => 'position',
	);
	...
	$settings[195] = array(
		...
		'type'		=> 'radio',
		'options' 	=> array(
			'bottom'=> __( 'Default (Natural Height)', 'ubermenu' ),
			'top' 	=> __( 'Top', 'ubermenu' ),
		),
		'default' 	=> 'bottom',
		'group'	=> 'position',
	...
	*/
	...
	$settings[300] = array(
		...
		'desc'		=> __( 'Set to "Unbounded" if you want a submenu wider than the menu bar. Only relevant for horizontally oriented menus.', 'ubermenu' ),
		'type'		=> 'radio',
		'default' 	=> 'on',
		'options'	=> [
			'on'	=> [
				'title' => __( 'Menu Bar' , 'ubermenu' ),
				'desc' => __('The submenu will be bound to the width of the menu bar.','ubermenu'),
			],
			'inner'	=> [
				'title' => __( 'Inner menu bar width' , 'ubermenu' ),
				'desc' => __('The submenu will be bound to the width of the inner menu bar.  See the Inner Menu Bar settings.','ubermenu'),
			],
			'off'	=> [
				'title' => __( 'Unbounded' , 'ubermenu' ),
				'desc' => __('UberMenu will not bound the submenu, but the theme might.  The submenu will be bound by the next relatively positioned ancestor element in your theme.','ubermenu'),
			],
		],
		'group'		=> array( 'position' , 'submenus' ),
		'customizer'=> true,
	...
	$settings[355] = array(
		...
		'desc' 		=> __( 'Select the default image position (can be overridden in Menu Item Settings for specific menu items).  Don\'t forget to set the Image Width appropriately if you use the Left or Right settings.', 'ubermenu' ),
		'type'		=> 'radio',
		'default'	=> ubermenu_new_default( $config_id , 'image_above' , 'image_left' ),
		'options'	=> [
			'image_left' => [
				'title' => __( 'Left', 'ubermenu' ),
				'desc' => __('Image appears on the left, text on the right', 'ubermenu'),
			],
			'image_above' => [
				'title' => __( 'Above' , 'ubermenu' ),
				'desc' => __('Image appears above the text', 'ubermenu'),
			],
			'image_right' => [
				'title' => __( 'Right' , 'ubermenu' ),
				'desc' => __('Image appears on the right, text on the left', 'ubermenu'),
			],
			'image_below' => [
				'title' => __( 'Below' , 'ubermenu' ),
				'desc' => __('Image appears below the text', 'ubermenu'),
			],
			'image_only' => [
				'title' => __( 'Image Only' , 'ubermenu' ),
				'desc' => __('Only the image will be displayed, no text', 'ubermenu'),
			],
		],
	...
	$settings[360] = array(
		...
		'desc' 		=> __( 'Set the title attribute on images', 'ubermenu' ),
		'type' 		=> 'checkbox',
		'default' 	=> 'off',
		'group'		=> 'images',
	);
	...
	$settings[361] = array(
		'name' 		=> 'image_alt_autofill',
		'label' 	=> __( 'Autofill Alt Attribute with Image Title', 'ubermenu' ),
		'desc' 		=> __( 'If the alt tag field for an image is empty, set the value to the image title.', 'ubermenu' ),
		'type' 		=> 'checkbox',
		'default' 	=> 'off',
		'group'		=> 'images',
	);
	...
	$settings[390] = array(
		...
		'desc' 		=> __( 'Don\'t display background images when on mobile devices', 'ubermenu' ),
		'type' 		=> 'checkbox',
		'default' 	=> 'off',
		'group'		=> 'images',
	);
	...
	$settings[480] = array(
		...
		'type'	=> 'header',
		'desc'	=> __( 'Visit the Theme Customizer to edit most of these settings with a Live Preview.' , 'ubermenu' ) . ' <a class="button button-tertiary" href="'.admin_url('customize.php').'"><i class="fas fa-pencil-alt"></i> '.__( 'Customizer', 'ubermenu' ).'</a>',
	...
	$settings[510] = array(
		...
		'type'	=> 'text',
		'desc'	=> __( 'Pixel value (do not include px)', 'ubermenu' ),
		'group'	=> 'style_customizations',
	...
	$settings[522] = array(
		...
		'type'	=> 'text',
		'desc'	=> __( 'A good way to increase or balance top level item heights, provided your text does not wrap.', 'ubermenu' ),
		'group'	=> 'style_customizations',
	...
	$settings[530] = array(
		...
		'options'	=> array(
			...
			'uppercase'	=> __( 'Uppercase', 'ubermenu' ),
			'capitalize'=> __( 'Capitalize', 'ubermenu' ),
		),
		'default'	=> '',
		'group'	=> 'style_customizations',
	...
	$settings[540] = array(
		...
		'options'	=> array(
			''			=> '&mdash;',
			'normal'	=> __( 'Normal', 'ubermenu' ),
			'bold'		=> __( 'Bold', 'ubermenu' ),
			'100'			=> '100',
			'200'			=> '200',
			'300'			=> '300',
			'400'			=> '400',
	...
	$settings[560] = array(
		...
		'type'	=> 'color',
		'desc'	=> __( 'Color of items that are active (hover/click depending on trigger)', 'ubermenu' ),
		'group'	=> 'style_customizations',
	...
	$settings[590] = array(
		...
		'type'	=> 'color_gradient',
		'desc'	=> __( 'Item background when activated (hover/click, depending on trigger)', 'ubermenu' ),
		'group'	=> 'style_customizations',
	...
	$settings[600] = array(
		...
		'type'	=> 'color_gradient',
		'desc'	=> __( 'Item background when current on the viewed page', 'ubermenu' ),
		'group'	=> 'style_customizations',
	...
	$settings[680] = array(
		...
		'type'		=> 'radio',
		'default'	=> 'edge',
		'options'	=> array( 'edge' => __( 'Edge', 'ubermenu' ) , 'text' => __( 'Text', 'ubermenu' ) ),
	...
	$settings[765] = array(
		...
		'options'	=> array(
			...
			'none'	=> 'None',
			'uppercase'	=> __( 'Uppercase', 'ubermenu' ),
			'lowercase'	=> __( 'Lowercase', 'ubermenu' ),
		),
	...
	$settings[810] = array(
		...
		'type'	=> 'color',
		'desc'	=> __( 'Color of the border below the header above the submenu stack', 'ubermenu' ),
		'group'	=> 'style_customizations',
	...
	$settings[820] = array(
		...
		'type'	=> 'radio',
		'default'	=> 'on',
		'options'	=> array( 'on' => __( 'On', 'ubermenu' ) , 'off' => __( 'Off', 'ubermenu' ) ),
		'desc'	=> __( 'Display a border below Column Headers with child items', 'ubermenu' ),
		'group'	=> 'style_customizations',
		'customizer'	=> true,
		'customizer_section' => 'headers',
	);
	...
	$settings[1161] = array(
		...
		'label' 	=> __( 'Default Badge Text Color' , 'ubermenu' ),
		'group'		=> 'badges',
		'type'		=> 'color',
		'customizer' => true,
		'customizer_section' => 'badges',
		'custom_style' => 'badge_text_color'
	);
	$settings[1162] = array(
		'name'		=> 'badge_background',
		'label' 	=> __( 'Default Badge Background Color' , 'ubermenu' ),
		'group'		=> 'badges',
		'type'		=> 'color',
		'customizer' => true,
		'customizer_section' => 'badges',
		'custom_style' => 'badge_background'
	);
	$settings[1163] = array(
		'name'		=> 'badge_font_size',
		'label' 	=> __( 'Badge Font Size' , 'ubermenu' ),
		'group'		=> 'badges',
		'type'		=> 'text',
		'customizer' => true,
		'customizer_section' => 'badges',
		'custom_style' => 'badge_font_size'
	);
	$settings[1165] = array(
		'name'		=> 'badge_border_radius',
		'label' 	=> __( 'Badge Border Radius' , 'ubermenu' ),
		'group'		=> 'badges',
		'type'		=> 'text',
		'customizer' => true,
		'customizer_section' => 'badges',
		'custom_style' => 'badge_border_radius'
	);
	$settings[1167] = array(
		'name'		=> 'badge_padding',
		'label' 	=> __( 'Badge Padding' , 'ubermenu' ),
		'group'		=> 'badges',
		'type'		=> 'text',
		'customizer' => true,
		'customizer_section' => 'badges',
		'custom_style' => 'badge_padding'
	);
	$settings[1168] = array(
		'name'		=> 'badge_font_weight',
		'label' 	=> __( 'Badge Font Weight' , 'ubermenu' ),
		'group'		=> 'badges',
		'type'	=> 'select',
		'options'	=> array(
			''			=> '&mdash;',
		...
		'customizer' => true,
		'customizer_section' => 'badges',
		'custom_style' => 'badge_font_weight'
	);

	$settings[1169] = array(
		'name'	=> 'badge_text_transform',
		'label'	=> __( 'Badge Text Transform' , 'ubermenu' ),
		'type'	=> 'select',
		'options'	=> array(
			''		=> '&mdash;',
			'none'	=> __( 'None', 'ubermenu' ),
		...
		'custom_style'	=> 'badge_text_transform',
		'customizer'	=> true,
		'customizer_section' => 'badges',
	);
...
function ubermenu_pro_customizer_sections( $wp_customize , $panel_id ){
	$wp_customize->add_section( $panel_id.'_badges', array(
		'title'		=> __( 'Badges', 'ubermenu' ),
		'priority'	=> 95,
		'panel'		=> $panel_id,
	) );
...
function ubermenu_export_data( $data ){
	...
	$html.= '<br/><span class="description">'.__( 'Copy and paste this text into an Import box to copy these settings to a different Configuration.', 'ubermenu' ).'</span>';
...
function ubermenu_import_data( $data ){
	...
	$html= '<a class="button button-primary" href="'.admin_url( 'themes.php?page=ubermenu-settings&amp;do=settings-import&amp;config_id='.$config_id .'&amp;ubermenu_nonce='.wp_create_nonce( 'ubermenu-control-panel-do' ) ).'">'.__( 'Import Settings', 'ubermenu' ).'</a>';
	return $html;
}
...
function ubermenu_settings_panel_fields_pro( $all_fields = array() ){
	...
	$fields[85] = array(
		'name' 		=> 'use_core_svgs',
		'label' 	=> __( 'Use Core SVGs for Essential Icons', 'ubermenu' ),
		'desc' 		=> __( 'By default, UberMenu will use Font Awesome icons for its essential icons (submenu indicator icons, responsive toggle icons).  Enable this setting to use built-in SVGs, or if you disable Font Awesome.', 'ubermenu' ),
		'type' 		=> 'checkbox',
		'default' 	=> 'off',
		'group'		=> 'assets',
	);
	...
	$fields[104] = array(
		...
		'desc' 		=> __( 'Enter your Google Maps API Key.  As of July 2018, this is required by Google.', 'ubermenu' ) . '  <a target="_blank" href="https://developers.google.com/maps/documentation/javascript/get-api-key">'.__( 'Get a key', 'ubermenu' ).'</a>',
		'type' 		=> 'text',
		'default' 	=> '',
		'group'		=> 'assets',
	);
	...
	$fields[105] = array(
		...
		'desc' 		=> __( 'It is generally recommended to let Google Maps determine this automatically for the user, but you can override the language here if you want to.  ', 'ubermenu'). ' <a target="_blank" href="https://developers.google.com/maps/documentation/javascript/localization">'.__('Google Maps Localization', 'ubermenu' ),
		'type' 		=> 'text',
		'default' 	=> '',
		'group'		=> 'assets',
	);
	...
	$fields[123] = array(
		'name'	=> 'mobile_menu_collapse_on_navigate',
		'label'	=> __( 'Collapse mobile menu on navigate' , 'ubermenu' ),
		'type'	=> 'checkbox',
		'default' => 'on',
		'desc'	=> __( 'Reset the mobile menu to the default closed state when the page is loaded / on user navigation.  Generally applicable when the user clicks the browser\'s "back" button after navigating via the expanded menu.', 'ubermenu' ),
		'group'	=> 'responsive',
	);
	...
	$fields[125] = array(
		...
		'options'	=> [
			'responsive'	=> [
				...
				'desc' => __('Display below responsive breakpoint', 'ubermenu'),
			],
			'mobile'		=> [
				'title' => __( 'Mobile' , 'ubermenu' ),
				'desc' => __('Use wp_is_mobile() mobile device detection', 'ubermenu'),
			],
			'touch'			=> [
				'title' => __( 'Touch Detection' , 'ubermenu' ),
				'desc' => __('Display when browser supports touch events', 'ubermenu'),
			],
		],
		'group'		=> 'responsive',
	);
	...
	$fields[294] = array(
		...
		'desc'	=> __( 'Settings to help tame misbehaving or complex themes.  See also: ' , 'ubermenu' ) . '<a href="https://sevenspark.com/docs/ubermenu-3/integration/theme-interference" target="_blank">'.__( 'Theme Interference', 'ubermenu' ).'</a>',
	);

	$fields[295] = array(
		'name'	=> 'theme_template_backtrace',
		'label'	=> __( 'Locate Theme Template Backtrace' , 'ubermenu' ),
		'desc'	=> __('Display an alert to admins above the menu to indicate which theme template the menu call originated from.  Useful for tracking down where to perform the manual integration if not directly in the header.php.  Disable when finished.' , 'ubermenu' ),
		'type'	=> 'checkbox',
		'default' => 'off',
	...
	$fields[297]	= array(
		...
		'type'	=> 'checkbox',
		'default'	=> 'on',
		'desc'	=> __( 'Sometimes theme filters will override UberMenu\'s filters, preventing UberMenu from properly integrating.  Enable this to try to force UberMenu\'s filters.  If there are no interfering filters, this can be disabled.', 'ubermenu' ),
		'group'	=> array( 'misc' , 'theme_integration' ),
	);
	...
	$fields[361] = array(
		'name'	=> 'delete_item_settings',
		'label'	=> __( 'Delete Menu Item Settings' , 'ubermenu' ),
		'desc'	=> '<a class="button button-primary" href="'.admin_url('themes.php?page=ubermenu-settings&do=delete-item-settings-check&ubermenu_nonce='.wp_create_nonce( 'ubermenu-control-panel-do' )).'">'.__( 'Delete Menu Item Settings' , 'ubermenu' ).'</a><br/><p>'.__( 'Delete ALL UberMenu menu item settings.  Will not delete menu items or core WordPress item settings.', 'ubermenu' ).'</p>',
		'type'	=> 'html',
...
function ubermenu_register_skins_pro(){
	...
	ubermenu_register_skin( 'white' , 			__( 'White', 'ubermenu' ) , $main.'white.css' );
	ubermenu_register_skin( 'deepsky' , 		__( 'Deep Sky', 'ubermenu' ) , $main.'deepsky.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'berry' , 			__( 'Berry', 'ubermenu' ) , $main.'berry.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'aqua' , 			__( 'Sea Green', 'ubermenu' ) , $main.'aqua.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'fire' , 			__( 'Fire', 'ubermenu' ) , $main.'fire.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'eggplant' , 		__( 'Eggplant', 'ubermenu' ) , $main.'eggplant.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'robinsegg' , 		__( 'Robin\'s Egg', 'ubermenu' ) , $main.'robinsegg.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'tangerine' , 		__( 'Tangerine', 'ubermenu' ) , $main.'tangerine.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'nightsky' , 		__( 'Night Sky', 'ubermenu' ) , $main.'nightsky.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'charcoal' , 		__( 'Charcoal', 'ubermenu' ) , $main.'charcoal.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'shinyblack' , 		__( 'Shiny Black', 'ubermenu' ) , $main.'shinyblack.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'simple-green' , 	__( 'Simple Green', 'ubermenu' ) , $main.'simplegreen.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'earthy' , 			__( 'Earthy', 'ubermenu' ) , $main.'earthy.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'black-silver' , 	__( 'Black & Silver', 'ubermenu' ) , $main.'blacksilver.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'blue-silver' , 	__( 'Blue & Silver', 'ubermenu' ) , $main.'bluesilver.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'red-black' , 		__( 'Red & Black', 'ubermenu' ) , $main.'redblack.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'orange' , 			__( 'Burnt Orange', 'ubermenu' ) , $main.'orange.css' , 'ubermenu-has-border' );
	ubermenu_register_skin( 'clean-white' , 	__( 'Clean White', 'ubermenu' ) , $main.'cleanwhite.css' );
	ubermenu_register_skin( 'trans-black' , 	__( 'Transparent Black', 'ubermenu' ) , $main.'trans_black.css' );
	ubermenu_register_skin( 'trans-black-hov' , __( 'Transparent Black - Hover', 'ubermenu' ) , $main.'trans_black_hover.css' );
	ubermenu_register_skin( 'silver-tabs' , 	__( 'Silver Tabs', 'ubermenu' ) , $main.'silvertabs.css' );

	ubermenu_register_skin( 'tt-silver' , 		__( 'Two Tone Silver & Black (Deprecated)', 'ubermenu' ) , $main.'twotone_silver_black.css' );
	ubermenu_register_skin( 'tt-black' , 		__( 'Two Tone Black & Black (Deprecated)', 'ubermenu' ) , $main.'twotone_black_black.css' );
	ubermenu_register_skin( 'tt-red' , 			__( 'Two Tone Red & Black (Deprecated)', 'ubermenu' ) , $main.'twotone_red_black.css' );
	ubermenu_register_skin( 'tt-blue' , 		__( 'Two Tone Blue & Black (Deprecated)', 'ubermenu' ) , $main.'twotone_blue_black.css' );
	ubermenu_register_skin( 'tt-green' , 		__( 'Two Tone Green & Black (Deprecated)', 'ubermenu' ) , $main.'twotone_green_black.css' );
	ubermenu_register_skin( 'tt-purple' , 		__( 'Two Tone Purple & Black (Deprecated)', 'ubermenu' ) , $main.'twotone_purple_black.css' );
	ubermenu_register_skin( 'tt-orange' , 		__( 'Two Tone Orange & Black (Deprecated)', 'ubermenu' ) , $main.'twotone_orange_black.css' );
	ubermenu_register_skin( 'tt-silver-s' , 	__( 'Two Tone Silver & Silver (Deprecated)', 'ubermenu' ) , $main.'twotone_silver_silver.css' );

	// Register 'None' at the end so that people stop picking this
	ubermenu_register_skin( 'none ' , '' , '' );
	ubermenu_register_skin( 'none' , __( 'None (disabled - *must provide your own custom skin*)', 'ubermenu' ) , '' );
...
function ubermenu_kb_search(){
	...
	<div class="ubermenu-kb-search">
		<div class="search-topper"><a target="_blank" href="<?php echo UBERMENU_KB_URL; ?>"><i class="fas fa-search"></i> <?php _e( 'Search the Knowledgebase', 'ubermenu' ); ?></a></div>
...
function ubermenu_support_forum_help(){
	...
	if( defined( 'UBERMENU_PACKAGED_THEME' ) ){
		...
		$html.= '<p>'.__( 'Didn\'t find the answer you needed in the Knowledgebase or Video Tutorials?  Visit the ' , 'ubermenu' ).
					'<a target="_blank" class="button" href="'.ubermenu_get_support_url().'"><i class="fas fa-life-ring"></i> '.__( 'Support Center', 'ubermenu' ).'</a></p>';
		$html.= '</div>';
	}
	return $html;
}
...
function ubermenu_extensions_section( $sections ){
	...
	$sections[] = array(
		...
		'sub_sections'	=> array(
			'ubermenu_extensions'	=> array(
				'title' 	=> __( 'UberMenu Extensions' , 'ubermenu' ),
			),
			'related_plugins'	=> array(
				'title' 	=> __( 'Related Plugins' , 'ubermenu' ),
			),
		),
	);
...
function ubermenu_extensions_fields( $fields = array() ){
	...
	$f[] = array(
			'name' 	=> 'um_extensions',
			'type' 	=> 'header',
			'label'	=> __('UberMenu Extensions' , 'ubermenu' ),
			'group'	=> 'ubermenu_extensions',
	);
	...
	$f[] = array(
			'name' 	=> 'um_related_plugins',
			'type' 	=> 'header',
			'label'	=> __('Related Plugins' , 'ubermenu' ),
			'group'	=> 'related_plugins',
	);

	$f[] = array(
			'name'	=> 'plugin_shiftnav',
			'label' => __( 'ShiftNav Mobile Menu' , 'ubermenu' ),
			'desc'	=> ubermenu_show_plugin_shiftnav(),
			'type'	=> 'html',
			'group'	=> 'related_plugins',
		);
	...
	$f[] = array(
			'name'	=> 'plugin_bellows',
			'label' => __( 'Bellows Accordion Menu' , 'ubermenu' ),
			'desc'	=> ubermenu_show_plugin_bellows(),
			'type'	=> 'html',
			'group'	=> 'related_plugins',
		);

	$f[] = array(
			'name'	=> 'plugin_duplicator',
			'label' => __( 'Carbon Copy Menu Duplicator' , 'ubermenu' ),
			'desc'	=> ubermenu_show_plugin_duplicator(),
			'type'	=> 'html',
			'group'	=> 'related_plugins',
		);
...
function ubermenu_show_plugin_shiftnav(){
	$html = '';
	...
	$html.= ubermenu_show_extension_info( array(
		'demo_url'	=> 'https://shiftnav.io/',
		'installed'	=> $installed,
		//'action'		=> $action,
		//'action_link' => $action_link,
		'img_src'	=> 'https://i.imgur.com/UqUMIUw.jpg',
		'purchase_url' => 'https://sevenspark.com/goods/shiftnav-pro',
		'description'	=> __( 'Enhance your mobile site with off-canvas navigation.' , 'ubermenu' ),
		'doc_url'	=> 'https://sevenspark.com/docs/shiftnav',
		'action_text' => __( 'Get the Plugin', 'ubermenu' ),
	));

	return $html;
}

function ubermenu_show_plugin_bellows(){
	$html = '';

	$installed = false;
	if( defined( 'BELLOWS_VERSION' ) ){
		$installed = true;
	}

	$html.= ubermenu_show_extension_info( array(
		'demo_url'	=> 'https://wpaccordionmenu.com/',
		'installed'	=> $installed,
		//'action'		=> $action,
		//'action_link' => $action_link,
		'img_src'	=> 'https://i.imgur.com/T3OIGRM.jpg',
		'purchase_url' => 'https://sevenspark.com/goods/bellows-pro',
		'description'	=> __( 'Complement your main navigation with a secondary accordion menu nav.' , 'ubermenu' ),
		'doc_url'	=> 'https://sevenspark.com/docs/bellows',
		'action_text' => __( 'Get the Plugin', 'ubermenu' ),
	));
...
function ubermenu_show_plugin_duplicator(){
	$html = '';

	$installed = false;
	if( defined( 'CARBON_COPY_MENU_DUPLICATOR_VERSION' ) ){
		$installed = true;
	}

	$html.= ubermenu_show_extension_info( array(
		'demo_url'	=> 'https://www.youtube.com/watch?v=2QmYWtR2-AM',
		'installed'	=> $installed,
		//'action'		=> $action,
		//'action_link' => $action_link,
		'img_src'	=> 'https://i.imgur.com/IrGlB3u.jpg',
		'purchase_url' => 'https://sevenspark.com/goods/carbon-copy-menu-duplicator',
		'description'	=> __( 'Duplicate and backup your menus, including UberMenu, ShiftNav, &amp; Bellows.' , 'ubermenu' ),
		'doc_url'	=> 'https://sevenspark.com/docs/carbon-copy-menu-duplicator',
		'action_text' => __( 'Get the Plugin', 'ubermenu' ),
	));
...
function ubermenu_show_extension_icons(){
	...
	$html.= ubermenu_show_extension_info( array(
		...
		'description'	=> __( 'Add over 1,500 Font Awesome Icons, customize color, size, layout, and more.' , 'ubermenu' ),
...
function ubermenu_show_extension_info( $info ){
	...
	$action_text = isset( $info['action_text'] ) ? $info['action_text'] : 'Get the Extension';
	...
	if( $info['installed'] ){
		...
		$html.= ' <a target="_blank" class="button button-primary" href="'.$info['purchase_url'].$analytics.'">'.$action_text.'</a>';
	}
...
function ubermenu_settings_welcome(){
	...
	<div class="ubermenu-welcome <?php if( !$show_welcome ) echo 'ubermenu-welcome-hide'; ?>">
		<div class="ubermenu-welcome-inner">
			<h2><?php _e( 'Welcome to UberMenu', 'ubermenu' ); ?> <?php echo UBERMENU_VERSION; ?></h2>
			...
			<div class="ubermenu-welcome-buttons">
				<a target="_blank" class="button button-primary" href="<?php echo UBERMENU_KB_URL; ?>"><i class="fas fa-book"></i> <?php _e( 'Knowledgebase', 'ubermenu' ); ?></a>
				<a target="_blank" class="button button-tertiary" href="<?php echo UBERMENU_VIDEOS_URL; ?>"><i class="fas fa-video"></i> <?php _e( 'Video Tutorials', 'ubermenu' ); ?></a>
				<a target="_blank" class="button button-secondary" href="<?php echo ubermenu_get_support_url(); ?>"><i class="fas fa-life-ring"></i> <?php _e( 'Support', 'ubermenu' ); ?></a>

				<span class="ubermenu-welcome-licenses"><?php _e( 'Your purchase entitles you to use UberMenu on one (1) site.', 'ubermenu' ); ?> <a target="_blank" class="ubermenu-license-info-link" href="http://goo.gl/lTLDhv"><i class="fas fa-question-circle"></i></a></span>
				<a target="_blank" class="button button-secondary button-new-license" href="https://wpmegamenu.com"><i class="fas fa-shopping-cart"></i> <?php _e( 'Need another license?', 'ubermenu' ); ?></a>
			</div>

			<p><?php _e( 'Links to the Knowledgebase, Video Tutorials, and Support Center will appear in the upper right of your Control Panel for easy access.  The QuickStart video below will help you get up and running quickly.', 'ubermenu' ); ?></p>
