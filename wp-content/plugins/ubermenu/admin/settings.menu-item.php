...
function ubermenu_admin_menu_load_assets()
{
	...
	wp_localize_script('ubermenu-menu-admin', 'ubermenu_meta', array(
		...
		'sandbox_url' => ubermenu_is_pro() ? ubermenu_sandbox_url() : false,
	)
	);
}
...
function ubermenu_menu_item_settings_panel()
{
	...
	// uberp( $ordered_panels );
	?>
	<div class="ubermenu-js-check">
		<div class="ubermenu-js-check-peek"><i class="fas fa-truck"></i> <?php _e('Loading UberMenu...', 'ubermenu'); ?>
		</div>
		<div class="ubermenu-js-check-details">
			<p>
				If this message does not disappear, it means that UberMenu has not been able to load.
				This most commonly indicates that you have a javascript error on this page, which will need to be resolved
				in order to allow UberMenu to run.
			...
			<div class="ubermenu-menu-item-panel-info">
				...
				<ul class="ubermenu-menu-item-tabs">
					<?php foreach ($ordered_panels as $order => $panel_id):
						...
						$icon = '';
						if (isset($panel['icon']))
							$icon = '<i class="fas fa-' . $panel['icon'] . '"></i> ';
						?>
						<li class="ubermenu-menu-item-tab" data-ubermenu-tab="<?php echo $panel_id; ?>"><a href="#"
								data-ubermenu-tab="<?php echo $panel_id; ?>"><?php echo $panel['title'];
								   echo $icon; ?></a></li>
					<?php endforeach; ?>
			...
			<div class="ubermenu-menu-item-panel-settings shift-clearfix">
				<form class="ubermenu-menu-item-settings-form" action="" method="post" enctype="multipart/form-data">
					...
					<div class="ubermenu-menu-item-save-button-wrapper">
						...
						<a class="ubermenu-menu-item-settings-close" href="#"><i class="fas fa-times"></i> <span
								class="ubermenu-key">ESC</span></a>
						...
						<a class="ubermenu-clear-settings">
							...
							<div class="ubermenu-menu-item-setting-tip ubermenu-menu-item-setting-tip-below"><i
									class="ubermenu-tip-icon fas fa-lightbulb"></i> <?php
							...
							<span class="ubermenu-menu-item-meta-ops-wrap">
								...
								<span class="ubermenu-menu-item-meta-ops">
									<span class="ubermenu-menu-item-setting">
										<label><input type="checkbox" name="ubermenu-meta-save-defaults" />
											<?php _e('Set as defaults', 'ubermenu'); ?></label>
										<div class="ubermenu-menu-item-setting-tip"><i
												class="ubermenu-tip-icon fas fa-lightbulb"></i>
											<?php
									...
									<span class="ubermenu-menu-item-setting">
										<label><input type="checkbox" name="ubermenu-meta-reset-defaults" />
											<?php _e('Reset defaults', 'ubermenu'); ?></label>
										<div class="ubermenu-menu-item-setting-tip"><i
												class="ubermenu-tip-icon fas fa-lightbulb"></i>
											<?php
					...
					<div class="ubermenu-menu-item-load-error-notice ubermenu-admin-notice ubermenu-admin-notice-warning">
						<p>If you are seeing this error, your settings have not loaded properly. This is most commonly
							caused by a memory exception
							from trying to load too many posts or terms. Please visit the <em>UberMenu Control Panel &gt;
								General Settings &gt; Advanced Menu Items</em> and enable the following settings:</p>
						<ul>
							<li><strong>Disable Autocomplete</strong></li>
							<li><strong>Disable Dynamic Posts Author Selection</strong></li>
						</ul>
						<p>If this does not resolve the issue, please enable WP_DEBUG and check for PHP errors in your
							Appearance &gt; Menus panel.</p>
					</div>
						...
						<div class="ubermenu-menu-item-tab-content" data-ubermenu-tab-content="<?php echo $panel_id; ?>" <?php
						   ...
						   ?>>
							...
							<div class="ubermenu-menu-item-tab-header">
								...
								<h3><?php echo $icon; ?>
									<!--<i class="fas fa-sliders"></i>--> <?php echo $panel['title']; ?> Settings
								</h3>
...
function ubermenu_show_menu_item_setting($setting, $default)
{
	...
	switch ($type) {
		case 'checkbox': ?>
			<input <?php echo $class; ?> type="checkbox" <?php echo "$name $data_setting";
				checked($default, 'on'); ?> />
			<?php break;
			...
			<div class="ubermenu-multicheck-wrap">
				...
				<?php foreach ($ops as $_val => $_name):
					...
					<label class="ubermenu-multicheck-label" title="<?php echo $_val; ?>">
						<input type="checkbox" class="checkbox ubermenu-multicheckbox" id="" <?php echo "$data_setting $_name_att $_value_att ";
						echo $checked; ?> />
						<?php echo $_name; ?>
					</label>
				<?php endforeach; ?>
		...
		case 'radio': ?>
			<div class="ubermenu-radio-group <?php if (isset($setting['type_class']))
				echo $setting['type_class']; ?> <?php if (count($ops) > 1)
					   echo 'ubermenu-radio-multiple-subgroups'; ?>">
				...
				<?php foreach ($ops as $_group_id => $group): ?>
					...
					<div class="ubermenu-radio-subgroup ubermenu-radio-subgroup-<?php echo $_group_id; ?>">
						...
						<?php foreach ($group as $_val => $_data):
							...
							$img_icon = '';
							if (isset($_data['img_icon']))
								$img_icon = $_data['img_icon'];
							?>
							<div class="ubermenu-radio-option">
								<label class="ubermenu-radio-label shift-clearfix<?php if ($img || $img_icon)
									echo ' ubermenu-radio-label-with-image'; ?><?php if ($selected)
										  echo ' ubermenu-radio-label-selected'; ?>">
			...
			<div class="ubermenu-icon-settings-wrap">
				...
				<div class="ubermenu-icons shift-clearfix">
					...
					<?php foreach ($ops as $_val => $data):
						if ($_val == '')
							continue; ?>
						<span class="ubermenu-icon-wrap" title="<?php echo $data['title']; ?>"
							data-ubermenu-search-terms="<?php echo strtolower($data['title']); ?>">
							<?php
							// uberp($_val);
							// group id::icon id format
							if (strpos($_val, '::')) {
								// $_val_arr = explode('::',$_val);
								// $group_id = $_val_arr[0];
								// $icon_id = $_val_arr[1];
			
								// echo ubermenu_generate_icon( $group_id, $icon_id, $data, ['data-ubermenu-icon'=>$_val]);
								echo ubermenu_generate_icon_markup($_val, ['data-ubermenu-icon' => $_val]);
								...
								// 			// uberp($data);
								// 			if( $data['src'] === 'inline' ){
								// 				echo '<span class="ubermenu-icon" data-ubermenu-icon="'.$_val.'">'
								// 					.$data['content'].'</span>';
								// 			}
								// 		}
								// 		break;
							...
							// Old FA System
								...
								?>
								<i class="ubermenu-icon <?php echo $_val; ?>" data-ubermenu-icon="<?php echo $_val; ?>"></i>
								<?php
							} ?>
						</span>
					<?php endforeach; ?>
					<span class="ubermenu-icon-wrap ubermenu-remove-icon" title="Remove Icon"><i class="ubermenu-icon"
							data-ubermenu-icon=""></i>Remove Icon</span>
				</div>
				<select <?php echo $class; ?> 			<?php echo "$name $data_setting"; ?>>
					<?php foreach ($ops as $_val => $data): ?>
						<option value="<?php echo $_val; ?>" <?php selected($default, $_val); ?>><?php echo $data['title']; ?>
						</option>
					<?php endforeach; ?>
				</select>
			</div>
		...
		case 'media': ?>
			<div class="ubermenu-media-wrapper">
				...
				<div class="ubermenu-media-buttons">
					<a class="ubermenu-setting-button" data-uploader-title="Upload or Choose from Media Library"><i
							class="fas fa-image"></i> Select</a>
		...
		case 'autocomplete': ?>
			...
			<div class="ubermenu-autocomplete">
				...
				<input class="ubermenu-autocomplete-input" data-auto-set="true" data-current-id="" data-current-name=""
				...
				<div class="ubermenu-autocomplete-ops">
					<?php foreach ($ops as $_val => $_name):
						...
						?>
						<span class="ubermenu-autocomplete-op" data-opname="<?php echo strtolower($_name); ?>"
							data-val="<?php echo $_val; ?>"><span class="ubermenu-autocomplete-op-name"><?php
...
function ubermenu_menu_item_settings()
{
	...
	$settings['row'][60] = array(
		...
		'title' => __('Row Column Default', 'ubermenu'),
		'type' => 'radio',
		'default' => 'auto',
		'desc' => __('The number of columns this row should be broken into by default.  Can be overridden on individual items', 'ubermenu'),
		'ops' => $column_ops,
	);
	...
	$settings['row'][67] = array(
		...
		'title' => __('Row Column Alignment', 'ubermenu'),
		'type' => 'radio',
		'default' => 'default',
		'desc' => __('Explicitly align the row columns to the left, right, or center, relative to the row container.  If in doubt, leave this set to default.', 'ubermenu'),
		'ops' => array(
			'group1' => array(
	...
	$settings['row'][70] = array(
		...
		'title' => __('Row Columns Dividers', 'ubermenu'),
		'type' => 'color',
		'default' => '',
		'desc' => __('The color of the border between each column in this row.  Note this only affects columns that are children of this item, not further descendants', 'ubermenu'),
	...
	$settings['row'][71] = array(
		...
		'title' => __('Row Columns Minimum Height', 'ubermenu'),
		'type' => 'text',
		'default' => '',
		'desc' => __('Useful when using the Row Columns Dividers setting', 'ubermenu'),
...
//die();
	...
	//Set Defaults
	if (
		isset($dirty_settings['ubermenu-meta-save-defaults']) &&
	...
	//Reset Defaults
	if (
		isset($dirty_settings['ubermenu-meta-reset-defaults']) &&
...
{
	return ubermenu_get_term_ops_by_tax(
		'category',
		array(
...
{
	return ubermenu_get_post_parent_ops(
		array(
