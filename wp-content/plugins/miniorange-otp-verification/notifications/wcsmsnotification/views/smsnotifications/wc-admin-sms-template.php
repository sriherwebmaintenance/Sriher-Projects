<?php
/**
 * View file to edit Admin SMS Template
 *
...
$prem_tags = explode( ',', $sms_settings->premium_tags );
...
echo '				<div>
						...
						echo '
							...
							<div class="w-full flex">
								<div class="flex-1">
									<h5 class="mo-title flex items-center">
										' . esc_html( 'Recipients' ) . '
									</h5>
	...
	echo '
			<div class="flex flex-col gap-mo-4 flex-1">
				<div class="mo-input-wrapper">
					<label class="mo-input-label"> Template Name </label>
					<input id="' . esc_attr( $template_name ) . '" name="' . esc_attr( $template_name ) . '" type="text" placeholder="Enter Template Name" value="' . esc_attr( $sms_settings->template_name ) . '" class="mo-input w-full secondary" />
				</div>
				<textarea  id="' . esc_attr( $variable_tag ) . '" class="mo-textarea w-full h-[128px]" rows="4"
				name="' . esc_attr( $variable_tag ) . '" placeholder="Enter sequence wise semicolon seperated tags used in message template. For eg. {Username}; {order-number}; {site-name}; {order-date};  " />' . esc_attr( $sms_settings->sms_tags ) . '</textarea>
...
if ( ! empty( $prem_tags[0] ) ) {
	echo '      <div class="w-full flex">
					<div class="flex-1">
						<h5 class="mo-title flex items-center gap-mo-2">' . wp_kses( $crown_icon, MoUtility::mo_allow_svg_array() ) . ' Premium Tags</h5>
						<p class="mo-caption mt-mo-2 pl-mo-6">Available in the <i><u><a href="' . esc_url( MOV_HOST ) . '/moas/login?redirectUrl=' . esc_url( MOV_HOST ) . '/moas/initializepayment&requestOrigin=wp_email_verification_intranet_woocommerce_plan" target="_blank">WooCommerce plan</a></u></i></p>
					</div>
					<div class="flex-1 mo-tags-section h-2/4">';
	foreach ( $prem_tags as $val ) {
		if ( 'billing tags:-' === $val || 'shipping tags:-' === $val ) {
			continue;
		}
		echo '			<div class="mo-tag bg-slate-100">' . esc_attr( $val ) . ' </div>';
	}
