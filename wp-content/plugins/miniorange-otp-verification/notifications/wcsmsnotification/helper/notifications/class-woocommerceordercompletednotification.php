<?php
/**
 * Helper functions for Woocommerce Order Completed Notifications
 *
 ...
 * to the WooCommerce Order Completed SMS Notification. It initializes the
 * notification related settings and implements the functionality for
 * sending the SMS to the user.
 */
if ( ! class_exists( 'WooCommerceOrderCompletedNotification' ) ) {
	/**
	 * WooCommerceOrderCompletedNotification class
	 */
	class WooCommerceOrderCompletedNotification extends SMSNotification {
		...
		protected function __construct() {
			...
			$this->page_header       = mo_( 'ORDER COMPLETED NOTIFICATION SETTINGS' );
			$this->page_description  = mo_( 'SMS notifications settings for Order Completion SMS sent to the users' );
		...
		public function send_sms( array $args ) {
			...
			$replaced_string = apply_filters( 'mo_wc_customer_order_completed_notif_string_replace', $replaced_string );
